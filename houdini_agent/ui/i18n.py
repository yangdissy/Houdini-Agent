...
_ZH = {
    ...
    # ===== Input Area =====
    'mode.tooltip': 'Agent：自主操作场景\nAsk：只读查询分析\nPlan：先生成计划，确认后执行',
    'confirm': '确认',
    'confirm.enabled': '执行前确认',
    'confirm.disabled': '免逐步确认',
    'confirm.tooltip': '点击切换执行确认方式\n\n执行前确认（默认，推荐）\n修改场景前显示预览并等待确认。\n\n免逐步确认（高风险）\n一般操作不再逐步询问，但不会绕过安全策略；特定高风险操作仍可能要求确认。',
    'guard.readonly': '只读',
    'guard.confirm': '需确认',
    'guard.high_risk': '高风险',
    'guard.readonly.tooltip': 'Ask 模式只读，不会修改 Houdini 场景。',
    'guard.confirm.tooltip': '修改场景前会请求确认。',
    'guard.high_risk.tooltip': '一般操作免逐步确认，但安全策略仍会拦截危险操作。',
    'safety.records': '安全记录 {}',
    'safety.attention': '{} 项需关注',
    'safety.tooltip': '查看本会话的操作确认、策略拦截和执行结果。',
    'context.add.tooltip': '添加图片或场景上下文',
    'context.attach_image': '附加图片…',
    'context.read_selection': '读取选中节点',
    'context.read_network': '读取当前网络',
    'context.auto_read': '自动读取上下文',
    'context.auto_read.off': '关闭',
    'context.auto_read.selection': '选中节点',
    'context.auto_read.network': '当前网络',
    'train.menu_label': '导出训练数据…',
    ...
    'ai.plan_mode_planning_prompt': (
        ...
        '## 计划质量标准\n\n'
        '### 步骤设计原则\n'
        '1. **按子系统/阶段拆 step,严禁按节点拆**。一个 step = 一个**完整的逻辑子系统**或 pipeline 阶段,\n'
        '   通常对应一次 `create_nodes_batch` 调用(可一次性创建该子系统所有节点+连接)。\n'
        '   ✗ 反例(每节点一步): step1 创建 Grid, step2 创建 Box, step3 创建 Scatter, step4 创建 CopyToPoints\n'
        '   ✓ 正例(子系统聚合): step1 "搭建散布系统(Grid+Box+Scatter+CopyToPoints,含全部连接)"——一次 batch 完成\n'
        '   判断标准: 如果两个相邻 step 都只创建/修改 1 个节点且依赖紧密,合并它们。\n'
        ...
        '4. **工具清单**:tools 必须列出该步骤要调用的具体工具名。**优先使用批量工具**:\n'
        '   `create_nodes_batch`(创建多节点+连接,首选)、`batch_set_parameters`(批量改参数)、\n'
        '   `layout_nodes`(整理布局)、`verify_network`(验证)。\n'
        '   只在确实只动 1 个节点时才用 `create_node` / `set_node_parameter`。\n\n'
        ...
        '### 复杂度匹配\n'
        '- 简单任务(改几个参数、搭建单个子系统):**1-3 步**。一个 4-6 节点的小网络通常 1 步搞定。\n'
        '- 中等任务(2-3 个子系统组成的效果):**3-5 步**,每步对应一个子系统。\n'
        '- 复杂任务(完整工作流,4-6 个子系统):**5-9 步**,按 Phase 分组。\n'
        '- 超复杂任务(整个项目):**10+ 步**,必须分 3-4 个 Phase。\n'
        '- 反模式检查:如果你的 step 数 ≥ 你要创建的节点数,你拆得太细了——合并成子系统。\n\n'
    ...
    'ai.plan_mode_execution_prompt': (
        ...
        '<plan_execution>\n'
        '你当前处于 **Plan 模式 — 执行阶段**。\n'
        '**这已经是执行模式，不是规划阶段。用户已确认计划，不需要也不允许再要求用户“切换到执行模式/Agent 模式”后才创建或修改节点。**\n'
        '**执行阶段拥有 Agent 执行工具权限：可以直接调用 `create_nodes_batch` / `create_node` / `connect_nodes` / `set_node_parameter` / `verify_network` 等工具。规划阶段的“禁止创建/修改节点”限制在这里不适用。**\n'
        ...
        '   - 若验证显示多个基础源节点/源 SOP 全部为空（例如 Grid/Box/Tube/Scatter 输入源都为 0），第一步必须检查 `update_mode` / `manual_mode`；若处于 Manual，不要先查参数 schema、改 generator 或重接源，直接按 Manual 刷新分支处理。\n'
        '   - 若工具结果包含 `validation_blocked=true` 且 `validation_block_reason="manual_empty_geometry_after_cook"`，这表示当前 Manual 模式下的空几何读数不可作为验证信号，不能当作几何真实为空的证据；必须执行 `recommended_next_action`，禁止继续用普通 cook、参数 schema 猜测或替换 generator 来验证。\n'
        '   - 如果工具结果或场景提示显示 Manual 模式：先用 `inspect_node` / `get_parameter_schema` / `verify_network` 排除连线和参数错误；仍为空时，直接调用 `set_update_mode(mode="auto")` 把 hip 切到 Auto Update，再重新验证几何。\n'
        '   - 当一个阶段因真正需要用户操作/授权（例如手动 cook、安装资产、选择视口对象）而无法继续时，不要把下游依赖步骤都标成最终失败；应先用 `ask_question` 暂停并等待用户决定。临时 Auto 几何验证不属于此类用户授权操作。\n'
    ...
    'ai.direct_execute_prompt': (
        ...
        '- 若 Houdini 处于 Manual 更新模式且验证显示 display 几何仍为空，调用 `set_update_mode(mode="auto")` 直接把当前 hip 切到 Auto Update 后重新验证；不要用 `execute_python` 手写 `hou.setUpdateMode`，也不要用 `ask_question` 请求切 Auto 授权。最终总结中说明已切到 Auto Update。\n'
...
_EN = {
    ...
    # ===== Input Area =====
    'mode.tooltip': 'Agent: autonomously operate the scene\nAsk: read-only query and analysis\nPlan: propose a plan before execution',
    'confirm': 'Confirm',
    'confirm.enabled': 'Confirm edits',
    'confirm.disabled': 'Skip step confirms',
    'confirm.tooltip': 'Toggle execution confirmation\n\nConfirm edits (default, recommended)\nShows a preview and waits before scene changes.\n\nSkip step confirms (high risk)\nRoutine actions proceed without step-by-step prompts, but safety policy remains active and may still require confirmation.',
    'guard.readonly': 'Read only',
    'guard.confirm': 'Confirm',
    'guard.high_risk': 'High risk',
    'guard.readonly.tooltip': 'Ask mode is read-only and cannot modify the Houdini scene.',
    'guard.confirm.tooltip': 'Confirmation is required before scene changes.',
    'guard.high_risk.tooltip': 'Routine actions skip step confirmations, but safety policy still blocks dangerous operations.',
    'safety.records': 'Safety log {}',
    'safety.attention': '{} need attention',
    'safety.tooltip': 'View confirmations, policy blocks, and execution results for this session.',
    'context.add.tooltip': 'Add an image or scene context',
    'context.attach_image': 'Attach Image…',
    'context.read_selection': 'Read Selection',
    'context.read_network': 'Read Current Network',
    'context.auto_read': 'Auto-read Context',
    'context.auto_read.off': 'Off',
    'context.auto_read.selection': 'Selection',
    'context.auto_read.network': 'Current Network',
    'train.menu_label': 'Export Training Data…',
    ...
    'ai.plan_mode_planning_prompt': (
        ...
        '1. **Group by subsystem/stage, NEVER by individual node**. One step = one **complete logical subsystem** or pipeline stage, '
        'typically corresponds to one `create_nodes_batch` call (all nodes + connections of that subsystem in one shot).\n'
        '   ✗ Anti-pattern (per-node steps): step1 create Grid, step2 create Box, step3 create Scatter, step4 create CopyToPoints\n'
        '   ✓ Correct (subsystem aggregation): step1 "Build scatter system (Grid+Box+Scatter+CopyToPoints with all connections)" — one batch\n'
        '   Heuristic: if two adjacent steps each only create/modify 1 node and are tightly coupled, MERGE them.\n'
        ...
        '4. **Tool manifest**: tools MUST list the specific tool names. **Prefer batch tools**: '
        '`create_nodes_batch` (multi-node + connections, first choice), `batch_set_parameters` (bulk params), '
        '`layout_nodes` (cleanup), `verify_network` (verify). '
        'Use `create_node` / `set_node_parameter` ONLY when truly touching a single node.\n\n'
        ...
        '### Complexity Matching\n'
        '- Simple (tweak params, build one subsystem): **1-3 steps**. A small 4-6 node network usually fits in 1 step.\n'
        '- Medium (2-3 subsystems): **3-5 steps**, one per subsystem.\n'
        '- Complex (full workflow, 4-6 subsystems): **5-9 steps**, grouped into phases.\n'
        '- Very complex (entire project): **10+ steps**, 3-4 phases.\n'
        '- Anti-pattern check: if your step count ≥ number of nodes you create, you split too finely — merge into subsystems.\n\n'
    ...
    'ai.plan_mode_execution_prompt': (
        ...
        '**This is already execution mode, not the planning phase. The user has confirmed the plan; you must NOT ask the user to switch to execution mode / Agent mode before creating or modifying nodes.**\n'
        '**The execution phase has Agent execution tool access: you may directly call `create_nodes_batch` / `create_node` / `connect_nodes` / `set_node_parameter` / `verify_network` and related tools. The planning-phase restriction against creating/modifying nodes does not apply here.**\n'
        ...
        '   - If validation shows that multiple base source nodes / all source SOPs are empty (for example Grid/Box/Tube/Scatter input sources all have 0 geometry), the first step MUST be checking `update_mode` / `manual_mode`; if Manual is active, do not inspect parameter schemas, replace generators, or reconnect sources first. Follow the Manual refresh branch.\n'
        '   - If a tool result contains `validation_blocked=true` and `validation_block_reason="manual_empty_geometry_after_cook"`, the empty geometry reading is not usable as a validation signal in the current Manual mode and must not be treated as evidence that geometry is truly empty; follow `recommended_next_action` and do not keep validating via ordinary cook, parameter-schema guessing, or generator replacement.\n'
        '   - If tool results or scene state indicate Manual mode: first rule out wiring/parameter problems with `inspect_node` / `get_parameter_schema` / `verify_network`; if still empty, call `set_update_mode(mode="auto")` to switch the current hip to Auto Update, then verify geometry again.\n'
        '   - When a step cannot continue because it genuinely needs user action/authorization (for example manual cook, install an asset, select viewport objects), do not mark all dependent downstream steps as final failures first; use `ask_question` to pause and wait for the user decision. Temporary Auto geometry validation is not such a user-authorization operation.\n'
    ...
    'ai.direct_execute_prompt': (
        ...
        '- If Houdini is in Manual update mode and verification still shows empty display geometry, call `set_update_mode(mode="auto")` to switch the current hip directly to Auto Update, then verify again; do not hand-write `hou.setUpdateMode` through `execute_python`, and do not use `ask_question` to request Auto-switch authorization. Mention in the final summary that the hip was switched to Auto Update.\n'
